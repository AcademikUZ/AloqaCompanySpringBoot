...
public class AuthService implements UserDetailsService {
    ...
    public ApiResult register(RegisterDto registerDto) {
        try {
            ...
            if(userInSystem.getRoles().getId() ==1 && registerDto.getRoleId()>7){
...
//            }
    ...
    public ApiResult edit(Long id, RegisterDto registerDto) {
        try {

            Optional<User> optionalUser = userRepository.findById(id);
            if(!optionalUser.isPresent())
                return new ApiResult("Bunday user yo'q", false);
            ...
            if (userInSystem != null) {
                if (userInSystem.getRoles().getId() >= registerDto.getRoleId()
                        || userInSystem.getId() != optionalUser.get().getId()
            ...
            if (sendMail)
                return new ApiResult("Muvoffaqiyatli tahrirlandi. Akkauntingiz aktivlatirish uchun emailingizni tasdiqlang " + savedUser.getEmail(), true);
    ...
    public ApiResult getOne(Long id) {
        ...
        if (userInSystem != null) {
            if (userInSystem.getRoles().getId() < optionalUser.get().getRoles().getId()
                    || userInSystem.getId() != optionalUser.get().getId()
            ) {
                return new ApiResult("Sizda bunday huquq yo'q", false);
            }
        ...
        return new ApiResult("OK", true, optionalUser.get());
